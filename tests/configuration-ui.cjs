  ...
  try {
    ...
    await page.evaluate(() => {
      const inventory = [
        ...
        ['packages/sauna.yaml', 'package', false],
      ...
      element.hass = { callWS: async msg => {
        ...
        if (msg.action === 'inspect') return { files: msg.files.filter(file => file.filename.endsWith('.yaml')).map(file => ({ ...file, path: file.filename === 'gasmeter.yaml' ? 'packages/gasmeter.yaml' : file.filename, mode: 'replace', source_matches: true, candidates: [] })) };
    ...
    await idle();
    assert.equal(await el('tree').locator('.file').count(), 5);
    ...
    console.log('Configuration UI: module loading, file tree, downloads, selection, merge availability, review invalidation, escaping, mobile layout, apply, restore and Assist navigation passed.');
  } finally {
