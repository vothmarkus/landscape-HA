...
def runtime(source: dict, monkeypatch):
    ...
    registry = SimpleNamespace(
        async_get=lambda _: entry, async_update_entity=update_entity
    )
    monkeypatch.setattr(assist_module, "Store", lambda *args, **kwargs: store)
    monkeypatch.setattr(
        assist_module, "collect_snapshot", lambda _: copy.deepcopy(current)
    )
    ...
    async def scenario():
        ...
        with pytest.raises(PatchError):
            await runtime.manager.async_apply(
                preview["preview_id"], selection, "admin-two"
            )
        assert not runtime.calls
        runtime.manager.discard(preview["preview_id"], "admin-one")
        with pytest.raises(PatchError):
            await runtime.manager.async_apply(
                preview["preview_id"], selection, "admin-one"
            )

    asyncio.run(scenario())


def test_only_selected_field_changes_and_sentinel_is_preserved(
    runtime, patch: dict
) -> None:
    async def scenario():
...
def test_concurrent_edit_after_preview_prevents_all_writes(
    runtime, patch: dict
) -> None:
    async def scenario():
        preview = await runtime.manager.async_preview(json.dumps(patch), "admin")
        ...
        with pytest.raises(PatchError):
            await runtime.manager.async_apply(
                preview["preview_id"],
                [item["id"] for item in preview["operations"]],
                "admin",
            )
        assert not runtime.calls
...
def test_failed_write_restores_names_alias_order_and_exposure(
    runtime, patch: dict
) -> None:
    async def scenario():
...
def test_baselines_survive_restart_and_unknown_sources_fail(
    runtime, patch: dict
) -> None:
    async def scenario():
